...
class BaseChatModel(ABC):
    ...
    def __init__(
        ...
        self.mcp_clients_connected_in_session = set()  # Track which clients we connected in this session
    ...
    async def aclear_registries(self) -> None:
        ...
        # Only close mcp clients that we connected in this session
        try:
            for client_name in self.mcp_clients_connected_in_session:
                if client_name in self.mcp_client_registry:
                    client = self.mcp_client_registry[client_name]
                    await client.aclose()
                    self.logger.info(f"Closed MCP client {client_name} connected in this session")
        finally:
            self.tool_registry.clear()
            self.mcp_client_registry.clear()
            self.mcp_clients_connected_in_session.clear()
        self.logger.info("Cleared tool registry and closed MCP clients connected in this session.")
    ...
    async def bind_mcp_client(self, client: MCPClient) -> List:
        ...
        if not client.is_connected():
            ...
            self.mcp_clients_connected_in_session.add(client.name)
            self.logger.info(f"Connected MCP client {client.name} in this session")
        else:
            self.logger.info(f"MCP client {client.name} already connected, reusing connection")
        ...
        try:
            if tool_name not in self.tool_registry:
                raise ToolNotFoundError(
                    "Unknown tool name: {}".format(tool_name)
                )
            ...
            try:
                if self.is_mcp_tool(tool):
                    out = await tool.provider.call_tool(
                        tool_name=tool.name, tool_args=params
                    )
                else:
                    if tool.is_coroutine:
                        out = await tool.func(**params)
                    else:
